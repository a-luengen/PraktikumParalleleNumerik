...
int main() {
 ...
 for(int j = 0; j < k; j++) {
   ...
   //Hij Schleife
   for(int i = 0; i<j-1;i++){ //muss in reihenfolge ablaufen
   ...
   if(testValue < 0) testValue = testValue * -1.0f;
...
//berechne y
 for(int i = k; i >= 0; i--){ //Muss in reihenfolge laufen
