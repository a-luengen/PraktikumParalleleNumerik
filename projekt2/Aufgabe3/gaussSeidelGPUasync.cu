...
__global__ void asyncLocalBlockRedBlack(int dim_u, int dim_u_emb, float h, float* u_emb, int iterCount) {
    
    int threadID, j_inner, i_emb, j_emb;
    float tempSum, newU;
    ...
    for(int i = 0; i < iterCount; i++) {
        // using jacobi iteration inside
        for(int iter_flag = 0; iter_flag < 2; iter_flag++) {
            threadID = threadIdx.x * 2 + iter_flag;
...
void gaussSeidel(int n, float fehlerSchranke, float h, float *u)
{
    ...
    while (fehlerSchranke < fehler)
    {
        ...
        asyncLocalBlockRedBlack<<<numBlocks, THREADS_PER_BLOCK>>>(n, n_emb, h, u_emb, 10);
